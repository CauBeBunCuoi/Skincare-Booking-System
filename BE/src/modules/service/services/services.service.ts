...
import {
  Service,
  ServiceModel,
} from 'src/database/schemas/service/service.schema';
...
import {
  ServiceStep,
  ServiceStepModel,
} from 'src/database/schemas/serviceStep/serviceStep.schema';
...
export class ServicesService {
  ...
  async addServiceType(serviceType: ServiceType): Promise<any> {
    ...
    try {
      const findLastServiceType =
        await this.serviceTypeRepository.findLastServiceType();
  ...
  async addService(service: {
    ...
    try {
      ...
      if (service.imageBase64) {
        ...
        const sourcePath = this.configService.get<string>(
          'imagePathConfig.SERVICE_IMAGE_PATH',
        );
  ...
  async filterServices(
    ...
    try {
      ...
      if (skinTypes.length === 0 && skinStatuses.length === 0) {
        const services =
          await this.serviceRepository.findByServiceTypeId(serviceTypeId);
        return await Promise.all(
          services.map(async (service) => {
            return {
              ...service,
        ...
        if (skinTypes.length === 0 && skinStatuses.length !== 0) {
          serviceSkinStatuses =
            await this.serviceSkinStatusRepository.findBySkinStatusIds(
              skinStatuses,
            );
          const serviceIds = serviceSkinStatuses.map(
            (serviceSkinStatus) => serviceSkinStatus.serviceId,
          );
          const servicesIdsByServiceType = await Promise.all(
        ...
        if (skinTypes.length !== 0 && skinStatuses.length === 0) {
          serviceSkinTypes =
            await this.serviceSkinTypeRepository.findBySkinTypeIds(skinTypes);
          const serviceIds = serviceSkinTypes.map(
            (serviceSkinType) => serviceSkinType.serviceId,
          );
          const servicesIdsByServiceType = await Promise.all(
            serviceIds.map(async (serviceId) => {
              const service = await this.serviceRepository.findById(serviceId);
  ...
  async getServiceDetail(serviceId: Types.ObjectId): Promise<{
    ...
    try {
      const service = await this.serviceRepository.findById(serviceId);
      const steps = await this.serviceStepRepository.findByServiceId(
        new Types.ObjectId(serviceId),
      );
      const serviceSkinTypes =
        await this.serviceSkinTypeRepository.findByServiceId(
          new Types.ObjectId(serviceId),
        );
      const serviceSkinStatuses =
        await this.serviceSkinStatusRepository.findByServiceId(
          new Types.ObjectId(serviceId),
        );
      const therapists = await Promise.all(
        (
          await this.therapistServiceRepository.findByServiceId(
            new Types.ObjectId(serviceId),
          )
        ).map(async (therapistService) => {
          const therapist = await this.accountRepository.findById(
            therapistService.accountId,
          );
          return {
            _id: therapist._id,
      ...
      return {
        ...
        skinTypes: await Promise.all(
          serviceSkinTypes.map(async (skinType) => {
            return await this.skinTypeRepository.findById(skinType.skinId);
          }),
        ),
        skinStatuses: await Promise.all(
          serviceSkinStatuses.map(async (skinStatus) => {
            return await this.skinStatusRepository.findById(
              skinStatus.statusId,
            );
          }),
        ),
        therapists: therapists.filter(
          (therapist) => therapist.isDeleted === false,
        ),
      };
    } catch (error) {
      console.log(error);
      throw new HttpException(
        'Get service detail failed',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async updateService(
    ...
    try {
      ...
      const serviceSteps = await this.serviceStepRepository.findByServiceId(
        new Types.ObjectId(serviceId),
      );
      ...
      await this.serviceStepRepository.deleteByServiceId(
        new Types.ObjectId(serviceId),
        session,
      );
  ...
  async deleteServiceType(serviceTypeId: number): Promise<any> {
    ...
    try {
      ...
      await this.serviceRepository.deleteByServiceTypeId(
        serviceTypeId,
        session,
      );
      await session.commitTransaction();
    ...
    return await Promise.all(
      serviceTypes.map(async (serviceType) => {
        return {
          ...serviceType,
          servicesCount:
            await this.serviceRepository.countServiceByServiceTypeId(
              serviceType._id,
            ),
        };
      }),
