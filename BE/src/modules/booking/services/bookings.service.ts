...
import {
  Booking,
  BookingModel,
} from 'src/database/schemas/booking/booking.schema';
...
import {
  WorkShift,
  WorkShiftModel,
} from 'src/database/schemas/workShift/workShift.schema';
...
export class BookingsService {
  ...
  get_StartTimeAndEndTime_ByDuration(
    duration: number,
  ...
  get_TimeArray(timeString: string): {
    hours: number;
    minutes: number;
    seconds: number;
  } {
  ...
  get_HourStrings_InPeriod(startHour: string, endHour: string): string[] {
    ...
    while (start.hours <= end.hours) {
      hours.push(
        `${this.padNumber(start.hours)}:${this.padNumber(start.minutes)}:${this.padNumber(start.seconds)}`,
      );
      start.hours += 1;
    }
    return hours;
  ...
  async get_AllWorkHours(): Promise<
    { workShiftId: number; workShiftHours: string[] }[]
  > {
    ...
    workShifts.forEach((workShift) => {
      if (
        !workShiftHours.some(
          (workShiftHour) => workShiftHour.workShiftId === workShift._id,
        )
      ) {
      ...
      while (start.hours <= end.hours) {
        const workShiftHour = workShiftHours.find(
          (workShiftHour) => workShiftHour.workShiftId === workShift._id,
        );
        workShiftHour.workShiftHours.push(
          `${this.padNumber(start.hours)}:${this.padNumber(start.minutes)}:${this.padNumber(start.seconds)}`,
        );
        start.hours += 1;
      }
  ...
  get_Next7DaySchedule_ByBookableHours(
    availableHours: { workShiftId: number; workShiftHours: string[] }[],
      ...
      .map((schedule) => {
        ...
        const availableHours = schedule.hours.filter((hour) => {
          return therapistFreeSchedules.some((free) =>
            free.freeSchedules.some(
              (freeSchedule) =>
                freeSchedule.date === schedule.date &&
                freeSchedule.hours.includes(hour),
            ),
          );
        });
    ...
    return therapistBusySchedules.map(({ therapistId, busySchedules }) => {
      let freeSchedules = next7DaySchedule.map(({ date, hours: busyhours }) => {
        let hours = busyhours.filter(
          ...
          (hour) =>
            busySchedules.every((unavailable) => {
              if (unavailable.date === date) {
                return (
                  !unavailable.hours.includes(hour) &&
                  !unavailable.hours.includes(
                    `${this.padNumber(this.get_TimeArray(hour).hours + duration - 1)}:00:00`,
                  )
                );
              }
              return true;
    ...
    return await Promise.all(
      bookings.map(async (booking): Promise<any> => {
        const bookingStatus = await this.bookingStatusRepository.findById(
          booking.bookStatusId,
        );
        const feedback = await this.feedbackRepository.findByBookingId(
          booking._id,
        );
        const executionResult =
          await this.executionResultRepository.findByBookingId(booking._id);
        const service = await this.serviceRepository.findById(
          booking.serviceId,
        );
        var therapist = null;
        if (booking.assignedTherapistId) {
          therapist = await this.accountRepository.findById(
        ...
        const result = {
          ...
          service: {
            ...service,
            imageUrl: await this.fileService.getImageUrl(
              this.configService.get<string>(
                'imagePathConfig.SERVICE_IMAGE_PATH',
              ),
              service._id,
              'main',
            ),
          ...
          therapist: therapist
            ? {
                ...
                imageUrl: await this.fileService.getImageUrl(
                  this.configService.get<string>(
                    'imagePathConfig.ACCOUNT_IMAGE_PATH',
                  ),
                  therapist._id,
                  'main',
                ),
  ...
  async addBooking(
    ...
    try {
      ...
      const progress = this.get_StartTimeAndEndTime_ByDuration(
        service.duration,
      ...
      const addedBooking = await this.bookingRepository.create({
        ...
        assignedTherapistId: booking.assignedTherapistId
          ? new Types.ObjectId(booking.assignedTherapistId)
          : null,
        extraFee: 0,
  ...
  async getBookingsByAccountId(accountId: Types.ObjectId): Promise<any> {
    const bookings = await this.bookingRepository.findByAccountId(
  ...
  async getBookingDetail(bookingId: Types.ObjectId): Promise<any> {
    ...
    return {
      ...
      service: {
        ...service,
        imageUrl: await this.fileService.getImageUrl(
          this.configService.get<string>('imagePathConfig.SERVICE_IMAGE_PATH'),
          service._id,
          'main',
        ),
      },
  ...
  async getAllSchedule(serviceId: Types.ObjectId): Promise<{
    ...
    try {
      ...
      const therapists_available =
        await this.therapistServiceRepository.findByServiceId(serviceId);
      ...
      const bookings_assigned = await this.bookingModel
        .find({
      ...
      const therapists_assigned = bookings_assigned.filter(
        (booking) => booking.assignedTherapistId,
      );

      // Lấy các therapist unassigned
      const therapists_unassigned = therapists_available.filter(
        (therapist) =>
          !therapists_assigned.some((assigned) =>
            assigned.assignedTherapistId.equals(therapist.accountId),
          ),
      );
      ...
      bookings_assigned.forEach((booking) => {
        ...
        if (
          !therapistBusySchedules.some((unavailable) =>
            unavailable.therapistId.equals(therapistId),
          )
        ) {
        ...
        const therapist = therapistBusySchedules.find((unavailable) =>
          unavailable.therapistId.equals(therapistId),
        );
        ...
        if (!busySchedules.some((busySchedule) => busySchedule.date === date)) {
          ...
          const busySchedule = busySchedules.find(
            (busySchedule) => busySchedule.date === date,
          );
      ...
      await Promise.all(
        therapistBusySchedules.map(async (therapist) => {
          const restSchedules =
            await this.restScheduleRepository.findByTherapistId(
              therapist.therapistId,
            );
          await Promise.all(
            restSchedules.map(async (restSchedule) => {
              const date = restSchedule.restDate.toISOString().split('T')[0];
              const workShift = await this.workShiftModel
                .findById(restSchedule.workShiftId)
                .exec();
              let hours = this.get_HourStrings_InPeriod(
                workShift.startHour,
                workShift.endHour,
              ...
              if (
                ...
                const busySchedule = therapist.busySchedules.find(
                  (busySchedule) => busySchedule.date === date,
                );
                hours = hours.filter(
                  (hour) => !busySchedule.hours.includes(hour),
                );
                busySchedule.hours = busySchedule.hours.concat(hours);
              }
            }),
      ...
      await Promise.all(
        therapists_unassigned.map(async (therapist) => {
          if (
            !therapistBusySchedules.some((unavailable) =>
              unavailable.therapistId.equals(therapist.accountId),
            )
          ) {
            therapistBusySchedules.push({
          ...
          await Promise.all(
            therapistBusySchedules.map(async (therapist) => {
              ...
              await Promise.all(
                restSchedules.map(async (restSchedule) => {
                  const date = restSchedule.restDate
                    .toISOString()
                    .split('T')[0];
                  const workShift = await this.workShiftModel
                    .findById(restSchedule.workShiftId)
      ...
      workHours.forEach((workHour) => {
        ...
        workHour.workShiftHours = workHour.workShiftHours.filter(
          (time, index) => {
      ...
      var next7DaySchedule =
        this.get_Next7DaySchedule_ByBookableHours(workHours);
      ...
      // })
      // console.log("----------------------------------------------------------------------------------");
      const therapistFreeSchedules = this.get_Next7DayTherapistFreeSchedules(
        therapistBusySchedules,
        next7DaySchedule,
        serviceDuration,
      );
      ...
      const availableTherapists = await Promise.all(
        therapistFreeSchedules
          .filter((therapist) => therapist.freeSchedules.length > 0)
          .map(async (therapist) => {
      ...
      return {
        schedules: this.filter_Next7DaySchedule_ByTherapistFreeSchedules(
          next7DaySchedule,
          therapistFreeSchedules,
        ),
        therapistFreeSchedules,
  ...
  async getScheduleByTherapistId(
    serviceId: Types.ObjectId,
    therapistId: Types.ObjectId,
  ): Promise<any> {
    try {
      ...
      const therapistFreeSchedule = therapistFreeSchedules.find((therapist) =>
        therapist.therapistId.equals(therapistId),
      );
      return {
  ...
  async addFeedback(
    bookingId: Types.ObjectId,
    feedback: { feedbackContent: string; rate: number },
  ): Promise<any> {
    await this.getExistBookingById(bookingId);
    try {
