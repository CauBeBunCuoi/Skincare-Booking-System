...
export class BookingRepository {
  ...
  async findByAssignedTherapistId(
    therapistId: Types.ObjectId,
  ): Promise<Booking[]> {
    return this.bookingModel
      .find({ assignedTherapistId: new Types.ObjectId(therapistId) })
      .exec();
  }
  ...
  async updateStatus(
    ...
    return this.bookingModel
      .findByIdAndUpdate(id, { bookStatusId: status })
      .exec();
  }
  ...
  async assignTherapist(
    ...
    await this.bookingModel
      .findByIdAndUpdate(bookingId, { bookStatusId: 3 })
      .exec();
    return this.bookingModel
      .findByIdAndUpdate(bookingId, {
        isAssigned: true,
        assignedTherapistId: new Types.ObjectId(therapistId),
      })
      .exec();
  }
